...
const Notification = () => {
  ...
  useEffect(() => {
    if (notificationData) {
      setNotificationSettings(prev => prev?.map((setting, index) => {
        // Update each section based on the API data
        if (index === 0) { // Email Notifications
          return {
            ...setting,
            isEnabled: notificationData.email_subscribed || false,
            subSettings: setting.subSettings.map(sub => {
              if (sub.id === "news-updates") {
                return { ...sub, isChecked: notificationData.news_and_updates_subscribed || false }
              }
        ...
        if (index === 1) { // Account Updates
          return {
            ...setting,
            subSettings: setting.subSettings.map(sub => {
              if (sub.id === "profile-changes") {
                return { ...sub, isChecked: notificationData.profile_update_notification_subscribed || false }
              }
              if (sub.id === "security-alerts") {
                return { ...sub, isChecked: notificationData.security_alerts_subscribed || false }
              }
              return sub;
            })
          }
        }
        if (index === 2) { // Transaction Alerts
          return {
            ...setting,
            subSettings: setting.subSettings.map(sub => {
              if (sub.id === "deposit-alerts") {
                return { ...sub, isChecked: notificationData.deposit_alerts_subscribed || false }
              }
              if (sub.id === "withdrawal-alerts") {
                return { ...sub, isChecked: notificationData.withdrawal_alerts_subscribed || false }
              }
              return sub;
            })
          }
        }
        if (index === 3) { // Trading Signals
          return {
            ...setting,
            subSettings: setting.subSettings.map(sub => {
              if (sub.id === "trading-signals") {
                return { ...sub, isChecked: notificationData.trading_signals_alerts_subscribed || false }
              }
              if (sub.id === "market-trends") {
                return { ...sub, isChecked: notificationData.market_trends_alerts_subscribed || false }
              }
              return sub;
            })
          }
        }
        if (index === 4) { // System Notifications
          return {
            ...setting,
            subSettings: setting.subSettings.map(sub => {
              if (sub.id === "maintenance" || sub.id === "upgrades") {
                return { ...sub, isChecked: notificationData.system_upgrade_alerts_subscribed || false }
              }
              return sub;
            })
          }
  ...
  const handleSaveChanges = async () => {
    ...
    const payload = {
      ...
      news_and_updates_subscribed: notificationSettings[0].subSettings.find(s => s.id === "news-updates")?.isChecked || false,
      newsletter_subscribed: notificationSettings[0].subSettings.find(s => s.id === "newsletter")?.isChecked || false,
      profile_update_notification_subscribed: notificationSettings[1].subSettings.find(s => s.id === "profile-changes")?.isChecked || false,
      security_alerts_subscribed: notificationSettings[1].subSettings.find(s => s.id === "security-alerts")?.isChecked || false,
      deposit_alerts_subscribed: notificationSettings[2].subSettings.find(s => s.id === "deposit-alerts")?.isChecked || false,
      withdrawal_alerts_subscribed: notificationSettings[2].subSettings.find(s => s.id === "withdrawal-alerts")?.isChecked || false,
      trading_signals_alerts_subscribed: notificationSettings[3].subSettings.find(s => s.id === "trading-signals")?.isChecked || false,
      market_trends_alerts_subscribed: notificationSettings[3].subSettings.find(s => s.id === "market-trends")?.isChecked || false,
      maintenance_alerts_subscribed: notificationSettings[4].subSettings.find(s => s.id === "maintenance")?.isChecked || false,
      system_upgrade_alerts_subscribed: notificationSettings[4].subSettings.find(s => s.id === "upgrades")?.isChecked || false,
    };
  ...
  return (
    <div className='w-full mx-auto rounded-xl p-8'>
     ...
     {isLoading?<div className='flex justify-center items-center py-8'><SmallSpinner color='white'/></div>: <div className="space-y-6">
      ...
      <div className="flex justify-start gap-4 mt-8">
        ...
        <Button className="px-6 py-2 rounded-lg flex justify-center items-center gap-x-4 bg-white text-[#2B3AA6] 
